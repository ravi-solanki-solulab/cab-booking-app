...
export class AppService {
  ...
  async nearbyCabs(nearbyCabDto: NearbyCabDto): Promise<NearbyCabRo[]> {
    ...
    const pipeline: any = [
      {
        $geoNear: {
          near: {
            type: 'Point',
            coordinates: [nearbyCabDto.longitude, nearbyCabDto.latitude],
          },
    ...
    const result: any[] = await this.connection.db
      .collection(COLLECTION_NAMES.CABS)
      .aggregate(pipeline)
      .toArray();
  ...
  async bookCab(bookingReq: BookingReqDto, userId: string): Promise<string> {
    const cab = await this.connection.db
      .collection(COLLECTION_NAMES.CABS)
      .findOne({ _id: new ObjectId(bookingReq.cabId) });

    //Check id cab is AVAILABLE to accept the booking request
    if (cab.status != CAB_STATUS.AVAILABLE) {
      throw new BadRequestException(
        'cab is not in AVAILABLE to accept the booking request',
      );
    }

    const pickup = this.GetGeoLocation(
      bookingReq.pickup.latitude,
      bookingReq.pickup.longitude,
    );
    const drop = this.GetGeoLocation(
      bookingReq.drop.latitude,
      bookingReq.drop.longitude,
    );

    //Inserting booking request in 'bookinga' collection
    const sendBooking = await this.connection.db
      .collection(COLLECTION_NAMES.BOOKINGS)
      .insertOne({
        userId: userId,
  ...
  async pastBookings(userId: string): Promise<BookingRo[]> {
    ...
    const pipeline: any = [
...

      {
        $lookup: {
          ...
          localField: 'cabId',
          pipeline: [
            { $project: { _id: 1, driverName: 1, contact: 1, title: 1 } },
          ],
          foreignField: '_id',
          as: 'cabDetails',
        },
    ...
    const bookingData: BookingRo[] = await this.connection.db
      .collection(COLLECTION_NAMES.BOOKINGS)
      .aggregate()
      .toArray();
